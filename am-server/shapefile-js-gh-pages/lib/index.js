'use strict';
var proj4 = require('proj4/dist/proj4-src.js');
var unzip = require('./unzip');
// added thenfs, so need to add this with npm install --save then-fs
...
function getZip(base, whiteList) {
	//changed to thenfs
	return thenfs.readFile(base).then(function(a) { 
	...
	if (typeof base === 'string') {
		if (base.slice(-4) === '.zip') {
			...
			return Promise.all([
				Promise.all([
					//changed to thenfs
					thenfs.readFile(base + '.shp'), 
					thenfs.readFile(base + '.prj')
				]).then(function(args) {
					return parseShp(toArrayBuffer(args[0]), args[1] ? proj4(args[1].toString()) : false);
				}),
				//changed to thenfs and parse.Dbf to shp.parseDbf.
				thenfs.readFile(base + '.dbf').then(shp.parseDbf) 
			]).then(shp.combine);
		}
	}
	else {
